...
本机没有 `Developer ID Application` 身份和 Apple 公证凭据，所以本地 DMG 使用 `CSC_IDENTITY_AUTO_DISCOVERY=false` 生成，只用于结构和功能验收。正式发布必须由 CI 注入签名/公证 secrets，并对两个架构执行 `codesign`、Gatekeeper 和 stapler 验证。自动更新仍使用 GitHub provider；发布 metadata 必须同时列出 arm64/x64 ZIP，CircleCI 已拆分架构安装并新增 metadata 合并步骤。
